package com.cryingonion.instashop.instagram;

public class InstagramConstants {

public static final String TAG = "InstagramConstant";
	
	public static final String CLIENT_ID = "0b6d7a8a06ed4d138ccf519887b6e439";
	public static final String CLIENT_SECRET = "51c3ece66ce9459da4386c7193362a69";
	
	// redirect uri
	public static final String CALLBACK_URL = "oauth2client://cryingonion";
	
	public static final String IG_REQUEST = "ig_request";	
	public static final int IG_REQ_LOGIN = 100;
	public static final int IG_REQ_USER_INFO = 101;
	public static final int IG_REQ_USER_FEEDS = 102;
	public static final int IG_REQ_USER_FOLLOWS = 106;
	public static final int IG_REQ_GET_CMNTS = 103;
	public static final int IG_REQ_POST_LIKE = 104;
	public static final int IG_REQ_POST_CMNT = 105;
	
	public static final String IG_MEDIA_VIDEO = "video";
	
}
